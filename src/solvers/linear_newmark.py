...
class LinearNewmarkSolver(Solver):
    def __init__(self, structure, neumann_BC, initial_conditions):
        super(LinearNewmarkSolver, self).__init__(structure, neumann_BC=neumann_BC, initial_conditions=initial_conditions)
    ...
    def run(self, beta1=1.0/2, beta2=1.0/2, t0=0, dt=1e-3, n_timesteps=1000, n_modes=10, verbose=True):
        ...
        for ii in range(1, n_timesteps + 1):
            ...
            vector3_ii = np.dot(Drom, self.mat_qV[:, ii - 1] + dt * self.mat_qA[:, ii - 1] / 2)
